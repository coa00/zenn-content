...
増えた理由ははっきりしていて、**1台で全部やろうとすると詰まった**からです。Claude Code をはじめ AI を裏で何本も走らせると、メモリがごっそり持っていかれて、肝心の業務側がもたつく。エディタもブラウザも重くなって、「AI を回すと自分の作業が止まる」という本末転倒な状態に・・。それで、重い AI 処理は別の Mac に `ssh` で入ってそっちで回す、という分担にしました。手元のメイン機は普段の作業に専念させて、AI は隣の Mac に逃がす。そうやって使う Mac が自然と増えていったわけです。

で、デスクのメイン機、持ち出し用の MacBook、AI を回す検証機——どの Mac でも Claude Code を同じように動かすとなると、地味に困るのが **トークンの扱い** です。Notion や Linear、GitHub の MCP サーバーを動かすには API トークンが要る。それを `.mcp.json` に直接書くと平文が設定ファイルに残るし、かといって3台に手でコピペして回るのもしんどい。
